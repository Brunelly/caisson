...
/// <summary>
/// NFR1's automated build-failing guard: the correlation engine must be pure and perform no I/O. This is
/// enforced statically by asserting the shipped <c>Caisson.Correlation</c> assembly references no EF Core,
/// no Npgsql, and no <c>System.Net.Http</c> — mirroring the driver/domain reflection guards.
/// </summary>
public sealed class PurityGuardTests
{
    private static readonly Assembly CorrelationAssembly = typeof(ITopologyCorrelationEngine).Assembly;
    ...
    {
        foreach (var reference in CorrelationAssembly.GetReferencedAssemblies())
        {
    ...
    public void No_referenced_assembly_pulls_in_database_or_http_io(string referenceName)
    {
        ForbiddenReferences.Should().NotContain(
            ...
            "Caisson.Correlation must stay pure/side-effect free (NFR1); {0} implies I/O", referenceName);
    }
