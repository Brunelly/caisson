...
/// <summary>
/// CWE-295: the Redfish TLS peer-certificate policy is never a silent accept-all. It accepts a fully
/// trusted chain, a matching SHA-256 fingerprint pin, or an explicit per-connection opt-in — and rejects an
/// untrusted certificate otherwise. Ported from the MikroTik driver's equivalent, verbatim policy.
/// </summary>
public sealed class TlsCertificateValidationTests
{
    ...
    public void A_fully_valid_chain_is_accepted_even_without_a_pin_or_opt_in()
    {
        ...
        using var client = ClientFor(SettingsWith(thumbprint: null, allowUntrusted: false));
    ...
    public void A_matching_fingerprint_pin_accepts_the_self_signed_certificate()
    {
        ...
        using var client = ClientFor(SettingsWith(thumbprint: pin, allowUntrusted: false));
    ...
    public void A_mismatched_fingerprint_pin_rejects_the_certificate()
    {
        ...
        using var client = ClientFor(SettingsWith(thumbprint: wrongPin, allowUntrusted: false));
    ...
    {
        using var cert = SelfSigned();
        using var client = ClientFor(SettingsWith(thumbprint: null, allowUntrusted: true));
    ...
    private static RedfishClient ClientFor(RedfishConnectionSettings settings)
        => new(settings, NullLogger.Instance);

    private static RedfishConnectionSettings SettingsWith(string? thumbprint, bool allowUntrusted)
        => new("192.0.2.1", 443, "user", "pass", TimeSpan.FromSeconds(2), thumbprint, allowUntrusted);

    private static X509Certificate2 SelfSigned()
    {
        using var rsa = RSA.Create(2048);
        var request = new CertificateRequest("CN=ilo.test", rsa, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
