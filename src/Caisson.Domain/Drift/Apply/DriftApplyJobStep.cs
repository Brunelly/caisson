...
/// <summary>
/// One durable step within a <see cref="DriftApplyJob"/> (story #65, AC4) — mirrors
/// <c>Discovery.DiscoveryJobStep</c> field-for-field. A mutable registry-style row: its status is
/// transitioned in place as the runner executes it, so a restarted process can resume from the last
/// completed step. Carries only counts/diagnostics in <see cref="ResultSummaryJson"/> — never secrets or
/// raw device data (NFR4).
/// </summary>
public sealed class DriftApplyJobStep
{
    ...
    private DriftApplyJobStep()
    {
        // EF Core materialization constructor.
    }

    /// <summary>Creates a pending step for a job.</summary>
    public DriftApplyJobStep(Guid id, Guid jobId, DriftApplyStepName stepName)
    {
        Id = id;
        JobId = jobId;
        StepName = stepName;
        Status = DriftApplyStepStatus.Pending;
    }
    ...
    public DriftApplyStepName StepName { get; private set; }

    /// <summary>Current durable status of the step.</summary>
    public DriftApplyStepStatus Status { get; private set; }

    /// <summary>Number of execution attempts made so far (bounded-retry).</summary>
    ...
    /// <summary>Wall-clock duration of the completed step in milliseconds.</summary>
    ...
    {
        Status = DriftApplyStepStatus.InProgress;
    ...
    public void Succeed(DateTime finishedAtUtc, string? resultSummaryJson = null)
    {
        ...
        Status = DriftApplyStepStatus.Succeeded;
    ...
    {
        Status = DriftApplyStepStatus.Failed;
        Finish(finishedAtUtc);
        ErrorCode = errorCode;
        ErrorMessage = Truncate(errorMessage);
    }

    /// <summary>Marks the step as skipped (already done, or the job reached a terminal state).</summary>
    public void Skip(DateTime finishedAtUtc)
    {
        Status = DriftApplyStepStatus.Skipped;
        Finish(finishedAtUtc);
    }
